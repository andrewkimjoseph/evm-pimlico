...
if (!process.env.NEXT_PUBLIC_PIMLICO_API_KEY) {
  throw new Error("NEXT_PUBLIC_PIMLICO_API_KEY is not defined");
}

// const PRIVATE_KEY = process.env.PRIVATE_KEY as Address;

// const PRIVATE_ACCOUNT = privateKeyToAccount(PRIVATE_KEY);
const NEXT_PIMLICO_API_KEY = process.env.NEXT_PUBLIC_PIMLICO_API_KEY ;

// console.log("NEXT_PIMLICO_API_KEY", NEXT_PIMLICO_API_KEY);

const SELECTED_CHAIN = celoAlfajores;

const PIMLICO_URL = `https://api.pimlico.io/v2/${SELECTED_CHAIN.id}/rpc?apikey=${NEXT_PIMLICO_API_KEY}`;
